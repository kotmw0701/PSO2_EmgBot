			...
			if(command.equalsIgnoreCase("help")) {
				...
				if(args.length == 2) {
					if(!NumberUtils.isNumber(args[1]) || (Integer.valueOf(args[1]) > 10 || Integer.valueOf(args[1]) < 1)) {
						sendMessageandTimeRemover(channel, second, "1-10の範囲で入れてください");
						return;
					}
					server = Integer.valueOf(args[1]);
				}
				sendMessageandTimeRemover(channel, second, (server+" 鯖 : "+ "**"+EmgHistory.getInstance().getEmergency(server)+"**"));
			} else if(command.equalsIgnoreCase("startup") || command.equalsIgnoreCase("shutdown")) {
				if(args.length != 2)
					return;
				if(!isActiveArks(msg)) {
					sendMessageandTimeRemover(channel, second, "権限所持者以外実行不可能です");
					return;
				}
				int cooltime = canRun(command);
				if(cooltime > 0) {
					String time = (cooltime/60)+"分"+(cooltime%60)+"秒";
					sendMessageandTimeRemover(channel, second, "クールタイムが終わっていません\r\n【残り "+time+" 】");
					return;
				}
				if(!NumberUtils.isNumber(args[1]) || (Integer.valueOf(args[1]) > 10 || Integer.valueOf(args[1]) < 1)) {
					sendMessageandTimeRemover(channel, second, "1-10の範囲で入れてください");
				...
				if(BotConfiguration.getToken(server) == null) {
					sendMessageandTimeRemover(channel, second, "対象のBotのTokenが設定されていないので、操作することが出来ません","管理者に問い合わせてください");
					return;
				}
				if(!BotClientManager.changeEnable(Integer.valueOf(args[1]), enable)) {
					sendMessageandTimeRemover(channel, second, (enable ? "稼働中です" : "稼働してません"));
					return;
				}
				Platform.runLater(() ->FxControllers.getServer(server).togglechange());
				sendMessageandTimeRemover(channel, second, server+"鯖のBotを"+(enable ? "稼働" : "停止")+"させました");
				...
				if(applicationId == null) {
					sendMessageandTimeRemover(channel, second, "対象のBotを立ち上げてから再試行してください");
					return;
				}
